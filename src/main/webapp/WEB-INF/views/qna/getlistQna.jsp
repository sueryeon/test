	...
	<div class="container-fluid p-3 pt-4 bg-light text-dark pb-5">
		<div class="wrap-container d-flex justify-content-between">
                     ...
                     <div class="qnaservice-box">
                        ...
                        <div class="card qna-service-box mt-2">
                        	<ul class="list-group list-group-flush">
							    <li class="list-group-item al-boardform"><a class="qna-alink" href="<%=contextPath%>/board/listBoards.do">게시판</a></li>
							    <li class="list-group-item al-qnaform active"><a class="qna-alink" href="<%=contextPath%>/qna/getlistQna.do">QnA</a></li>
						   </ul>    
                        </div>
                     </div>
                  </div>
			...
			<div class="right ms-4 text-center" style="width: 80%;" >
				<h3 class="border-bottom border-2 border-secondary-subtle text-start pb-3 mt-4 pt-5 mb-0"><strong>Q&A</strong></h3>
				<table class="table">
					<thead>
						<tr>
							<th scope="col" colspan="4" class="text-end" >
								<a href="<%=contextPath%>/qna/getqnaForm.do" class="btn btn-primary me-4">글쓰기</a>
							</th>
						</tr>
						<tr>
							<th class="col-sm-2" scope="col">문의현황</th>
					...
					<tbody>
						<c:choose>
							<c:when test="${empty qnaMap.AllListQna}">
								<tr>
							...
							<c:when test="${!empty qnaMap.AllListQna}">
								<c:forEach var="list_Qna" items="${qnaMap.AllListQna}" >
									<tr>
										<td>공백</td> 
										<th class="text-start">
											<a href="<%=contextPath%>/qna/getDetailQna.do?qarticleno=${list_Qna.qarticleno}" class="write-link">${list_Qna.qtitle}</a>
										</th>
										<td>${list_Qna.qid}</td>
										<td><fmt:formatDate value="${list_Qna.qwritedate}" pattern="yy-MM-dd" /> </td>
									</tr>
								</c:forEach>
							</c:when>
						</c:choose>
						<tr>
							<td colspan="4">
								<c:if test="${qnaMap.totalArticles !=null}">
									<c:choose>
										<c:when test="${qnaMap.totalArticles > 100}">
											<c:forEach var="page" begin="1" end="10" step="1">
												<c:if test="${qnaMap.pagesize>1&&page==1}">
													<a class="no_uline ms-2 me-2" href="<%=contextPath%>/qna/getlistQna.do?pagesize=${qnaMap.pagesize-1}&pagenum=${(qnaMap.pagesize-1)*10+1}">
														&nbsp; pre
													</a>
												</c:if>
													<a class="no_uline ms-2 me-2" href="<%=contextPath%>/qna/getlistQna.do?pagesize=${qnaMap.pagesize}&pagenum=${page}">
														${page}
													</a>
												<c:if test="${page==10}">
													<a class="no-uline ms-2 me-2" href="<%=contextPath%>/qna/getlistQna.do?pagesize=${qnaMap.pagesize+1}&pagenum=${qnaMap.pagesize*10+1}">
														&nbsp; next
													</a>
												</c:if>
											</c:forEach>
										...
										<c:when test="${qnaMap.totalArticles < 100}">
											<c:forEach var="page" begin="1" end="${qnaMap.totalArticles/10+1}" step="1">
												<c:choose>
													<c:when test="${page==qnaMap.pagenum}">
														<a class="sel-page ms-2 me-2 active" href="<%=contextPath%>/qna/getlistQna.do?pagesize=${qnaMap.pagesize}&pagenum=${page}">
															<span style="font-weight: bold;">${page}</span> 
														</a>
													</c:when>
													<c:otherwise>
														<a class="sel-page ms-2 me-2" href="<%=contextPath%>/qna/getlistQna.do?pagesize=${qnaMap.pagesize}&pagenum=${page}">
															${page}
														</a>
													</c:otherwise>
												</c:choose>
