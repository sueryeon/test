...
//------------------------------------------------------------------------병원검색 js
	...
	function fn_step2_pre(){
		...
		if(mid!=null||mid.length!=0){
			obj.action = "${contextPath}/reservation/getlistMyReservations.do";
		      obj.submit();
		}
		else if(adid!=null||adid.lenghth!=0){
			obj.action = "${contextPath}/reservation/getlistReservations.do";
		      obj.submit();
		}
						...
						<c:when test="${!empty rsvVO.rsubname}">
							<div class="d-flex">
								<div class="form-check">
								  <input class="form-check-input" type="radio" name="radio" id="radio1" onchange="showDiv()" disabled>
								  <label class="form-check-label" for="radio1">
								     본인 예방접종 예약
								  </label>
								</div>
								<div class="form-check">
								  <input class="form-check-input" type="radio" name="radio" id="radio2" onchange="showDiv()" disabled checked>
					...
					<c:choose>
						<c:when test="${!empty rsvVO.rsubname}">
							...
							<main class="form-sign mt-4">
								<form action="<%=request.getContextPath()%>/reservation/updateReservation.do?action=family" method="post">
